...
if(isset($_POST['roomTypeName'])){
    ...
    $fileName1 = $_FILES['roomTypeImage1']['name'];
    $fileName2 = $_FILES['roomTypeImage2']['name'];
    $tempname = $_FILES['roomTypeImage']['tmp_name'];
    $tempname1 = $_FILES['roomTypeImage1']['tmp_name'];
    $tempname2 = $_FILES['roomTypeImage2']['tmp_name'];
    $folder = "../assets/picture/RoomType/";

    // Check if all images are selected
    if (!empty($fileName) && !empty($fileName1) && !empty($fileName2)) {
        // Move uploaded images to the destination folder
        if(move_uploaded_file($tempname, $folder . $fileName) && move_uploaded_file($tempname1, $folder . $fileName1) && move_uploaded_file($tempname2, $folder . $fileName2)) {
            // Insert room type details into the database
            $insert_query = "INSERT INTO room_type (RoomType, RoomImage, RoomImage1, RoomImage2, Cost, Description) VALUES ('$roomType', '$fileName', '$fileName1', '$fileName2', '$roomCost', '$desc')";
        ...
        $error = "Please select all three images";
        $sendData = array(
            "msg"=>"",
            "error"=>$error
        );
...
//update the details of the room type
if(isset($_POST['roomTypeId'])){
    ...
    $fileName1 = $_FILES['editRoomTypeImage1']['name1'];
    $fileName2 = $_FILES['editRoomTypeImage2']['name2'];
    $tempname = $_FILES['editRoomTypeImage']['tmp_name'];
    $tempname1 = $_FILES['editRoomTypeImage1']['tmp_name1'];
    $tempname2 = $_FILES['editRoomTypeImage2']['tmp_name2'];
    $folder = "../assets/picture/RoomType/";
    ...
    if (!empty($fileName) && !empty($fileName1) && !empty($fileName2)) {
        ...
        if(move_uploaded_file($tempname, $folder . $fileName) && move_uploaded_file($tempname1, $folder . $fileName1) && move_uploaded_file($tempname2, $folder . $fileName2)) {
            ...
            $update_query = "UPDATE room_type SET RoomType='$roomType', RoomImage='$fileName', RoomImage1='$fileName1', RoomImage2='$fileName2', Cost='$roomCost', Description='$desc', Status='$status' WHERE RoomTypeId='$updateId'";
            ...
            if(mysqli_query($con, $update_query)){
                ...
                $error = "Error in Updating ...! Try after sometime";
                $sendData = array(
                    "msg"=>"",
                    "error"=>$error
                );
        ...
        // If all images are not selected, update the room type details without changing the images
        $update_query = "UPDATE room_type SET RoomType='$roomType', Cost='$roomCost', Description='$desc', Status='$status' WHERE RoomTypeId='$updateId'";
...
if(isset($_POST['roomTypeId'])){
    ...
    if ($num_rooms==0) {
        ...
        if($nums>0){
            ...
            $update="UPDATE room_type SET  RoomType='$roomType', RoomImage ='$fileName',RoomImage1='$fileName1', RoomImage2='$fileName2', Description='$desc',Status='$status',Cost='$roomCost' where RoomTypeId = '$updateId'" ;
...
if(isset($_POST['eventTypeUpdateId'])){
  ...
  //update the details of event type table
